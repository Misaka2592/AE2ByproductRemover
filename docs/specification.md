...
- 模式由世界的服务端配置统一控制，默认独立计算；保存当前世界的配置文件后自动重载，无须重新进入世界。重载完成后创建的合成计算使用新值，已经开始的计算保留创建时的模式。已生成的预览不自动重算，确认下单后仍按原计划执行；运行中及存档恢复的任务继续保留原计划与进度。
...
| 世界保持加载，保存配置在独立计算与复用模式间双向切换 | 文件监听触发重载，新建计算分别采用 2A／1A 语义，无须重新进入世界 |
| 已创建计算或已生成预览后保存新配置 | 原计算与预览保留旧模式；预览不自动重算，点击开始及之后存档恢复均保留原计划，新计算使用新模式 |
...
配置热重载仅支持本分支的 NeoForge 1.21.1，复用加载器的文件监听、校验与 `ModConfigEvent.Reloading`。NeoForge 的 `config/fml.toml` 中 `disableConfigWatcher` 需为默认值 `false`。仅当前模组的 SERVER 配置事件更新模式；原生及 Thunderbolt 计算继续使用创建时的模式快照。客户端不负责决定服务端的计算模式，不新增管理员命令或同步协议。Forge 1.20.1 不加入热重载，保持重新加载世界或重启服务器后生效。
...
- NeoForge 21.1.250 / AE2 19.2.17：29 项基础测试通过，覆盖输出压紧、真实编码菜单、真实 AE2 规划、CPU 派发与存档恢复；包含输出排列一致性、复用模式保守检查及真正输入循环的回归场景。配置测试通过真实 FML 文件监听验证普通覆盖保存、原子替换的双向重载与缓存刷新；旧预览在切换模式后提交及恢复仍保留原计划。
...
加载这些真实附属 JAR 后，共 63 项测试通过：基础测试 29 项、Thunderbolt 7 项、Data Energistics 6 项、AdvancedAE／Neo ECO CPU 共 12 项、AdvancedAE 类型与方向编码保护 9 项。原生与 Thunderbolt 规划均验证双向切换配置后保留已创建计算的模式快照。修复前的回归测试分别复现了规划与重编码缺陷。
