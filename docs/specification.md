...
| P：A→B，Q：B→A，无启动库存，请求 B | 两种模式均不能凭空启动循环 |
...
- Forge 47.4.16 / AE2 15.4.10：4 项输出压紧测试、6 项真实世界 GameTest 通过，覆盖产物注册、2A／1A 规划、无用产物过滤、双目标任务恢复和保留加工次数；包含输出排列一致性、复用模式保守检查及真正输入循环的回归场景。
